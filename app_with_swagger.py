"""
Local Advanced Prompting System - Flask API with Swagger UI
Main Flask application entry point with integrated API documentation
"""
...
from swagger_config import create_api_models
from api_docs import create_documented_endpoints
...
def create_app():
    """Create and configure Flask application with Swagger UI"""
    app = Flask(__name__)
    ...
    # Setup Swagger UI
    try:
        swagger_config = create_api_models()
        swagger_api = swagger_config['api']
        swagger_blueprint = swagger_config['blueprint']
        swagger_models = swagger_config['models']
        
        # Create documented endpoints
        documented_api = create_documented_endpoints(swagger_api, swagger_models)
        
        # Register Swagger blueprint
        app.register_blueprint(swagger_blueprint, url_prefix='/api/v1')
        logger.info("Swagger UI configured successfully")
    except Exception as e:
        logger.error(f"Failed to setup Swagger UI: {e}")
    ...
    def root():
        ...
        welcome_data = {
            ...
            "base_url": base_url,
            "available_endpoints": {
                "swagger_documentation": f"{base_url}/docs/",
                "api_info": f"{api_prefix}/info",
                "health_check": f"{api_prefix}/health"
            }
        }
...
def main():
    ...
    logger.info(f"🚀 Swagger UI: http://{host}:{port}/api/v1/docs/")
